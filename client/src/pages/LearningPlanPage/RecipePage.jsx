...
const RecipePage = () => {
  const generatePDF = () => {
    ...
    doc.text(cuisine.name || "Cuisine", pageWidth / 2, bannerHeight / 2 + 8, {
      align: "center",
    });
    y = bannerHeight + 20;
    ...
    // 7. Save & Download
    doc.save(
      `${(cuisine.name || "Cuisine").replace(/\s+/g, "_")}_Recipe_Book.pdf`
    );
  };
  ...
  return (
    <>
      <div className="max-w-8xl mx-auto p-10">
        ...
        <div
          className={`min-h-screen bg-gradient-to-br from-${theme.color}-50 to-white p-10`}
        >
          {showConfetti && (
          ...
          {progress === 100 && (
            <div className="text-center mb-10 animate-bounce">
              <h2 className="text-3xl font-bold text-blue-600 drop-shadow-md">
                🎉 You Did It, Master Chef! 🎉
              </h2>
              <p className="text-gray-600 mt-2">
                Every recipe completed. Go treat yourself to dessert 😋
              </p>
            </div>
          )}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-8 justify-items-center">
            {recipes.map((recipe, idx) => (
              ...
              >
                <img
                  src={
                    recipe.image ||
                    "https://via.placeholder.com/400x200.png?text=Yummy+Dish"
                  }
                  alt={recipe.name}
                  className="w-full h-100 object-cover rounded-lg mb-4"
                />
                <h2 className={`text-3xl font-bold mb-5 ${theme.text}`}>
                  {recipe.name}
                </h2>
                <div className="mb-3">
                  <span className="text-sm text-gray-800">⏱️ Time:</span>{" "}
              ...
              <button
                ...
                className={`px-14 py-2 rounded-full bg-${theme.color}-500 text-white hover:bg-${theme.color}-600`}
              >

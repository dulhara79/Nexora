...
export default function ForumHomePage() {
  ...
  const fetchSavedQuestions = async () => {
    if (!isAuthenticated || !user || !token) {
      console.log("Not authenticated, resetting savedQuestionIds");
      setSavedQuestionIds([]);
      return;
    }
    try {
  ...
  const handleSaveToggle = async (questionId, isSaved) => {
    ...
    // Optimistic update
    const previousSavedQuestionIds = [...savedQuestionIds];
    setSavedQuestionIds((prev) =>
      isSaved ? prev.filter((id) => id !== questionId) : [...prev, questionId]
    );

    try {
      const endpoint = isSaved
        ? `/api/questions/saved-questions/${questionId}`
        : `/api/questions/saved-questions`;
      const method = isSaved ? axios.delete : axios.post;
      const data = isSaved ? {} : { questionId };

      const response = await method(`${BASE_URL}${endpoint}`, data, {
        headers: {
      ...
      console.log(`Save/Unsave Response for ${questionId}:`, response.data);
  ...
  const fetchCommunities = async () => {
    ...
    setCommunities([
      ...
      },
      {
        ...
        icon: "🥗",
        members: 3218,
        description: "Plant-based recipes and cooking tips.",
      },
    ]);
  };
  ...
  const handleTagSelect = (tag) => {
    setFilterOptions({
      ...filterOptions,
      tags: filterOptions.tags.includes(tag)

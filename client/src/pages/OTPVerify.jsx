...
const OTPVerify = () => {
  ...
  const handleOTPSubmit = async (e) => {
    ...
    try {
      ...
      login({ id: response.data.id, email: response.data.email, name: response.data.name });
      navigate("/feed");
    } catch (err) {
  ...
  return (
    <div className="flex items-center justify-center w-screen min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50">
      <motion.div ref={ref} variants={containerVariants} initial="hidden" animate={inView ? "visible" : "hidden"} className="w-full max-w-md px-4 py-12 sm:px-6 lg:px-8">
        <div className="p-8 border border-white shadow-2xl bg-white/50 backdrop-blur-lg rounded-3xl sm:p-10">
          ...
          <form onSubmit={handleOTPSubmit} className="space-y-6">
            <motion.div variants={itemVariants}>
              <label className="block mb-2 text-sm font-medium text-gray-700">One-Time Password (OTP)</label>
          ...
          <motion.div variants={itemVariants} className="mt-6 text-sm text-center text-gray-500">
            Didn’t receive an OTP?{" "}
            <button onClick={handleResendOTP} className="font-medium text-indigo-600 hover:text-indigo-500" disabled={loading || !email}>
              Resend OTP
            </button>
          </motion.div>
        </div>

...
// Icons
import { 
  House, ChefHat, BookOpen, Trophy, MessageSquareMore,
  User, Menu, X, Search, ChevronDown, ChevronUp, LogOut
} from "lucide-react";
...
const PAGE_THEMES = {
  ...
  post: {
    ...
    nav: "bg-gradient-to-r from-amber-500 to-yellow-500",
    text: "text-white",
    active: "bg-white/20",
    hover: "hover:bg-white/10",
    submenuBg: "bg-gradient-to-r from-amber-400/95 to-yellow-400/95",
    submenuText: "text-white",
    submenuHover: "hover:bg-white/20",
    submenuActive: "bg-white/30"
  },
  learn: {
    ...
    nav: "bg-gradient-to-r from-emerald-500 to-teal-500",
    text: "text-white",
    active: "bg-white/20",
    hover: "hover:bg-white/10",
    submenuBg: "bg-gradient-to-r from-emerald-400/95 to-teal-400/95",
    submenuText: "text-white",
    submenuHover: "hover:bg-white/20",
    submenuActive: "bg-white/30"
  },
  challenges: {
    ...
    nav: "bg-gradient-to-r from-purple-500 to-indigo-600",
    text: "text-white",
    active: "bg-white/20",
    hover: "hover:bg-white/10",
    submenuBg: "bg-gradient-to-r from-purple-400/95 to-indigo-500/95",
    submenuText: "text-white",
    submenuHover: "hover:bg-white/20",
    submenuActive: "bg-white/30"
  },
  forum: {
    ...
    active: "bg-white/20",
    hover: "hover:bg-white/10",
    submenuBg: "bg-gradient-to-r from-orange-500 to-amber-500",
    submenuText: "text-white",
    submenuHover: "hover:bg-white/20",
    submenuActive: "bg-white/30"
  },
...
export default function EnhancedHeader() {
  ...
  const handleScrollUpdate = (isScrolled) => {
    setHeaderState(prev => ({
      ...
      textColor: isScrolled 
        ? "text-gray-800" 
        : PAGE_THEMES[activeSection].text,
  ...
  const handleTabClick = (sectionKey) => {
    if (lastClickedTabRef.current === sectionKey && submenuOpen) {
      // Close submenu if clicking the same tab again
      setSubmenuOpen(false);
    } else {
      // Open submenu and update active section
  ...
  return (
    <>
            ...
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center space-x-2">
                <motion.div 
                  ...
                  className={`p-2 rounded-full ${headerState.isScrolled ? "bg-white/20" : "bg-white/10"}`}
                >
              ...
              <nav className="items-center hidden space-x-1 md:flex">
                ...
                {isAuthenticated ? (
                  <div className="relative group">
                    <motion.button
                      whileTap={{ scale: 0.95 }}
                      className={`flex items-center space-x-2 p-2 rounded-full ${headerState.hoverClass}`}
                    >
                      <FallbackAvatar 
                    ...
                    {/* Dropdown */}
                    <div className="absolute right-0 hidden w-48 py-2 mt-2 transition-all duration-300 origin-top-right transform scale-95 bg-white rounded-lg shadow-xl opacity-0 group-hover:block group-hover:opacity-100 group-hover:scale-100">
                      <Link to={`/profile/${user.id}`} className="block px-4 py-2 text-gray-800 hover:bg-gray-100">Profile</Link>
                      <Link to={`/edit/${user.id}`} className="block px-4 py-2 text-gray-800 hover:bg-gray-100">Settings</Link>
                      <button 
                        onClick={logout}
                  ...
                  <div className="flex items-center space-x-2">
                    <motion.button
                      ...
                      className="px-4 py-2 text-orange-500 transition-colors bg-white rounded-lg hover:bg-gray-100"
                    >
                      Login
                    </motion.button>
                    <motion.button
                      ...
                      className="px-4 py-2 text-white transition-colors bg-orange-500 rounded-lg hover:bg-orange-400"
                    >
                      Sign Up
                    </motion.button>
                  </div>
                  ...
                  {NAV_STRUCTURE[activeSection].submenu.map((item) => {
                    ...
                    return (
                      <motion.button
                        ...
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        className={`px-4 py-1.5 mx-1 rounded-lg transition-all duration-200 ${PAGE_THEMES[activeSection].submenuText} ${
                          isActive 
                    ...
                    return (
                      <div key={item.path} className="mb-2">
                        <div className="flex items-center">
                          <button
                            onClick={() => setSectionSubmenuOpen(!sectionSubmenuOpen)}
      ...
      <div className={`${submenuOpen ? "h-24" : "h-16"} transition-all duration-300`}></div>
    </>
  );
}